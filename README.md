...
The first HIL slice is bench-verified: the board beacons live telemetry, the host runner detects link loss against the 5 s heartbeat timeout, and both HIL scenarios pass with scope-backed packet timing. The ICM-20948 IMU is also in the loop: accel/gyro plus AK09916 magnetometer samples go out as timestamped telemetry with per-half validity flags, and the sensor monitor turns invalid or frozen sources into the matching dropout faults. `ACCEL_GYRO_DROPOUT` is Degraded and can retreat POINTING/DETUMBLE to STANDBY; `MAG_DROPOUT` is a report-only Warning. INA228 power telemetry is now live on the bench - bus voltage, current, and power go out as timestamped, validity-flagged samples, with the current reading verified against the bench draw; `POWER_DROPOUT` covers an invalid or missing monitor sample. Structural temperature (TMP117) is next.
