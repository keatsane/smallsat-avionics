...
- **Phase 8: the LoRa beacon transmits (2026-07-31).** SPI3 arbitration moved to a per-peripheral lock so the camera and both radios share the bus; the RFM95 driver brings up and answers; and the heartbeat now goes out over the air alongside both UARTs, byte-identical on all three. Both bench checks passed - `LORA: up`, and the payload downlink re-ran clean after the FIFO burst change. **Owed: reception** - there is no second radio to hear the beacon until the ground station exists, so REQ-TLM-004 stays in progress. The **nRF24 driver** landed too, carrying payload chunks in DOWNLINK - so the same frame now leaves over two uarts and two radios, all four bench-confirmed. **The ground station has started** in `gsw/`: an Adafruit Feather M0 RFM95, which replaces the Teensy the plan used to name. It compiles `common/protocol/` rather than reimplementing it, and its firmware is a byte pipe - a packet arrives, the bytes go out USB, and `uart_monitor.py` decodes them unchanged. **The beacon was received over the air on 2026-07-31** - `HEARTBEAT` at 1 Hz, decoded by the same tooling that reads the ST-Link cable, with no ground-side change. The radios also report their own health now (`LoraStatus`/`Nrf24Status`), which had been reserved since June and left empty. The **nRF24 receiver and the SSD1306 readout** landed the same day - the box now hears both radios, pumps both streams out one USB port, and shows mode, faults and link state with no PC attached. **Owed: the over-the-air image**, which is what closes REQ-TLM-004 with both transports demonstrated rather than one.
