...
- **Still owed from phase 5:** REQ-HMI-001's artifact - a photo or scope capture of the status array against the decoded heartbeat. And the ESC wire: the OBC sees zero bytes (`waiting=0 ore=0 fe=0 ne=0`), so PA10 must reach the ESC's TX (PB3 on J3) and PA9 its RX (PB4), crossed not straight, with the ESC's USB **out** (J3 and its ST-Link VCP are the same USART2) and a common ground. Suspect the cable's landing pins first. The `TEMP esc rx:` block in `obc/Src/main.cpp` prints every 2 s until the first status frame arrives; delete it once `ESC: up at N ms, flags=0x0F` appears.
...
- **Undervoltage -> SAFE demonstration.** Reachable now that bus voltage reads real battery state of charge - sweep the supply down past 13.6 V.
...
- **An `inhibits` bitmask in the heartbeat.** Reconnecting the console mid-run shows the two inhibited faults with the BENCH BUILD banner long out of scrollback and nothing on the wire saying their responses are suppressed - the LED bead distinguishes it, the telemetry does not. Same shape as the `faults` mask; it makes every saved log self-describing, which is what REQ-FAULT-012 needs of a reader who missed the banner.
- **Automate REQ-VV-004.** "Every requirement traces to a verifying artifact, kept current" cannot be checked by hand at 60 requirements. A script that parses `requirements.md` and asserts every named artifact exists would close it and catch drift, the same way the wire-format mirror tests already do.
